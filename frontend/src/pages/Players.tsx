  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex items-center justify-between mb-2 px-2">
        <h3 className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em]">Personnel Registry</h3>
        <span className="text-[10px] font-black text-violet-500/80 uppercase tracking-[0.2em]">{total} Players Found</span>
      </div>
      ...
      <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-xl shadow-xl overflow-hidden">
        <ul className="divide-y divide-zinc-800/50">
          {players.map(p => (
            <li key={p.guid}>
              <Link to={`/player/${p.guid}`} className="flex justify-between items-center px-6 py-4 hover:bg-zinc-800/40 transition duration-200 group">
                <div className="flex flex-col gap-0.5">
                  <span className="font-semibold tracking-wide text-zinc-200 text-lg group-hover:text-white transition-colors">
                    <QuakeName name={p.display_name} />
                  </span>
                  {p.main_role && (
                    <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">{p.main_role}</span>
                  )}
                </div>
